...
public:
    explicit
    decoder_state(recordio_protobuf_reader const &reader);

private:
    void
    init_state(schema const &shm);

    void
    init_tensor(feature_desc const &desc);

    void
    init_coo_tensor_builder(feature_desc const &desc);
...
init_tensor(feature_desc const &desc)
{
    std::size_t data_size = desc.shape()[0] * as_size(desc.strides()[0]);

    auto arr = make_cpu_array(desc.dtype(), data_size);

    auto tsr = make_intrusive<dense_tensor>(desc.shape(), std::move(arr));
...
init_coo_tensor_builder(feature_desc const &desc)
{
    auto bld = make_coo_tensor_builder(desc);
