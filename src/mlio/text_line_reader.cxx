...
text_line_reader::make_record_reader(data_store const &ds)
{
    ...
    auto rdr =
        make_intrusive<detail::text_line_record_reader>(std::move(strm), true);
    return std::move(rdr);
}

void
text_line_reader::reset() noexcept
{
    parallel_data_reader::reset();
}

void
text_line_reader::infer_schema(instance const &ins)
{
    // no op
}
...
{
    const std::vector<instance> &instances = batch.instances();
    ...
    descs.emplace_back(feature_desc_builder{
        ...
        data_type::string,
        {batch.size(), 1}}.build());
    intrusive_ptr<schema> schema_ = make_intrusive<schema>(descs);
    std::vector<intrusive_ptr<tensor>> tensor_vec;
    tensor_vec.emplace_back(tsr);
    return make_intrusive<example>(schema_, std::move(tensor_vec));
}
                              ...
                              std::size_t batch_size) const
{
    ...
    for (const instance &i : instances) {
        auto temp = as_span<char const>(i.bits());
        std::string text_line(temp.data(), 0, temp.size());
        string_vec.emplace_back(text_line);
    }
    auto arr = wrap_cpu_array<data_type::string, std::vector<std::string>>(
        std::move(string_vec));
