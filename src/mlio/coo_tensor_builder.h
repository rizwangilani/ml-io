...
public:
    explicit
    coo_tensor_builder(feature_desc const &desc)
        : desc_{&desc}, coordinates_(desc.shape().size())
    {}
...
public:
    explicit
    coo_tensor_builder_impl(feature_desc const &desc)
        : coo_tensor_builder{desc}
    {}
...
struct make_coo_tensor_builder_op {
    ...
    operator()(feature_desc const &desc)
    {
        return std::make_unique<coo_tensor_builder_impl<dt>>(desc);
    }
};

inline std::unique_ptr<coo_tensor_builder>
make_coo_tensor_builder(feature_desc const &desc)
{
    return dispatch<make_coo_tensor_builder_op>(desc.dtype(), desc);
}
